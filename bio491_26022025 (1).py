# -*- coding: utf-8 -*-
"""BIO491_26022025.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IUPi4qn-S3Dg8N79jiZlnDvC7b_HMoCP
...
def uniprot_query (entries_list, fields_list):
  ...
  for entry in entries_list: #entry is the uniprot accession
    ...
    if response.status_code == 200:
      ...
      protein_name = 'None'
      if result != {}:
        if 'proteinDescription' in result.keys():
...
def protein_interactors_counter (interactors_list, pairs_list):
  ...
  for pair in pairs_list:
    ...
    # since the interactors list are the query proteins that i used as my initial datasets I know that there are no 'None' values so is unesessary to check for 'None' values
    if interactor_a in interactors_list: # if the first interactor belongs to the interactors list i save the interactor
...
#Retrieve the data needed for analysis

!wget https://raw.githubusercontent.com/evasiliou/BIO491/refs/heads/main/Input%20Datasets/disprot.tsv
!wget https://raw.githubusercontent.com/evasiliou/BIO491/refs/heads/main/Input%20Datasets/uniprot.tsv
...
tsv_file = 'uniprot.tsv'
uniprot_df=pd.read_table(tsv_file, sep='\t')
# print (uniprot_df.head().to_string())
uniprot_acc = []
for i in uniprot_df.index:
  if uniprot_df.loc[i,'Entry'] not in uniprot_acc: #create a list that contains all the idr proteins as in disprot DB with no duplicates
      uniprot_acc.append(uniprot_df.loc[i,'Entry'])

print ('\n Total uniprot proteins: ', len(uniprot_acc))

#create a new file that contains only the uniprot ids without any duplicates
column_label = ['Uniprot ID']
column_data = [uniprot_acc]
# column_data.append(disprot_acc)
# print(column_data)
...
gene_enrichment(disprot_protein_genes_dict,'disprot_gene_enrichment.csv' )
...
gene_enrichment(uniprot_protein_genes_dict, 'uniprot_gene_enrichment.csv')
...
disprot_dict = read_gene_enrichment_results('disprot_gene_enrichment.csv')#this is a dictionary
...
for go_term in common_go_terms:
    ...
    common_genes = list(set(uniprot_genes).intersection(set(disprot_genes))) #this is a list

    if common_genes != {}: # check if the result of the intersection is not empty
        common_dict[go_term] = common_genes
    else:
...
#With the code below we create a new dictionary that has as keys the enriched genes and as values the go terms
...
column_label = ['Enriched Proteins', 'Enriched Genes']
column_data = [enriched_proteins, enriched_genes]
filename = 'enriched_proteins_genes.csv'
new_csv_file(column_label,column_data,filename,False)

"""#MultiUniRed Data Analysis"""

!wget https://raw.githubusercontent.com/evasiliou/BIO491/refs/heads/main/MultiUniRed%20Results/disprotVSuniprot/disVSuni_multi_query_output.tsv
!wget https://raw.githubusercontent.com/evasiliou/BIO491/refs/heads/main/MultiUniRed%20Results/uniProtVSdisProt/uniVSdis_multi_query_output.tsv

read_multiunired_results.counter = 0
uni_dis_pairs = read_multiunired_results('uniVSdis_multi_query_output.tsv', 1.0, 'proteins', False)
dis_uni_pairs = read_multiunired_results('disVSuni_multi_query_output.tsv', 1.0, 'proteins', False)

print (len(uni_dis_pairs))
print (len(dis_uni_pairs))

total_multiunired_pairs = pairs_intersection(uni_dis_pairs, dis_uni_pairs)
print (len(total_multiunired_pairs))
print (total_multiunired_pairs)

#save the whole list with the multiunired pairs in a txt file - it will be easier and faster to reload them
with open ('list_multiunired_pairs.txt', 'w') as txtfile:
  txtfile.write(str(total_multiunired_pairs))
...
batch_size = 200
fetched_items = 0  # the number of items that have been submitted
count_files = 0 #count the total number of the created files
...
while fetched_items < total_items:
      ...
      hippie_query_api(batch_proteins, 0.63, outfile_name) #submit a query in HIPPIE
...
total_hippie_pairs = []
excluded_files = [] # saves the filenames of files that they contain no results from HIPPIE analysis - this may occur because of an error (e.g. 500: Server Error)
...
for i in range(count_files):
  ...
  with open (input_filename, 'r') as tsvfile:
    ...
    if "No form found on the page." in file_content:
...
new_csv_file(column_labels = ['Interactor A', 'Interactor B', 'Confidence Value'], column_data = [df['Interactor A'], df['Interactor B'], df['Confidence Value']], filename = 'hippie_total_pairs_concat.csv', index_status = False )

#with the code below i obtain only the unique protein interactions from HIPPIE (it is a way to eliminate possible duplicates in the form of ('A','B') or ('B','A'))

total_hippie_pairs = [] #create a new list that contains the unique HIPPIE pairs
interactor_A_list = []

interactor_B_list = []
# i keep the df from the previous code cell since it contains all the hippie pairs - not necessary to read again the .csv file
for i in range(len(df)):
  interactor_a = df.loc[i,'Interactor A']
  interactor_b = df.loc[i,'Interactor B']
  pair = (interactor_a, interactor_b) #(e.g. ('A','B'))
  reversed_pair = tuple(reversed(pair)) #(e.g. ('B','A'))

  if interactor_a == interactor_b and pair not in total_hippie_pairs: #take the symmetric interactions (e.g. ('A','A'))
    total_hippie_pairs.append(pair)
    interactor_A_list.append(interactor_a)
    interactor_B_list.append(interactor_b)

  elif pair not in total_hippie_pairs and reversed_pair not in total_hippie_pairs:
    total_hippie_pairs.append(pair)
    interactor_A_list.append(interactor_a)
    interactor_B_list.append(interactor_b)
...
new_csv_file(column_labels = ['Interactor A', 'Interactor B'], column_data = [interactor_A_list, interactor_B_list], filename = 'hippie_total_unique_pairs.csv', index_status = False )

# if (reversed(('P21675', 'P20226'))) in total_hippie_pairs:
#   print ('True')

#create a text file that contains the list with the pairs - it is easier and takes much less time to reload the protein interactions from HIPPIE instead of creating again the list

with open ('list_hippie_pairs.txt', 'w') as txtfile:
  txtfile.write(str(total_hippie_pairs))
...
total_interactors_list = mobidb_query_proteins
total_mobidb_dict = {}
...
while fetched_items < total_items:
      ...
      mobidb_dict = mobidb_query(batch_proteins, 'proteins') #submit a query in MobiDB
...
not_found_mobidb = list(set(mobidb_query_proteins).difference(set(total_mobidb_dict.keys())))

with open ('not_found_mobidb.txt', 'w') as f:
  for protein in not_found_mobidb:
    f.write(f'{protein} \n')
...
"""#Networks Generation - Case Studies"""

# import networkx as nx

# def new_ppi_network(nodes, edges, filename, database):
...
#   nx.draw(g, pos, with_labels=True, font_weight='bold', node_color = color)
#   plt.savefig(filename)
#   plt.show()
...
#before to create the networks i decided to convert all the interactors (query proteins and their interactors from both HIPPIE and MultiUniReD) to the equivalent gene names and then save them to a csv file
...
for pair in total_hippie_pairs:
  ...
  if interactor_a not in all_proteins:
    all_proteins.append(interactor_a)

  if interactor_b not in all_proteins:
    all_proteins.append(interactor_b)
...
#find the equivalent gene names
proteins_to_genes_dict = protein_to_gene_conversion (all_proteins)
print (len(proteins_to_genes_dict))
print (proteins_to_genes_dict)

#find the equivalent Protein Names
uniprot_protein_name_dict = uniprot_query(all_proteins, ['protein_name'])
print (len(uniprot_protein_name_dict))
print (uniprot_protein_name_dict)

proteins = [] # a list to save the uniprot accession numbers
genes = [] # a list to save the gene names
protein_names = [] # a list to save the protein names

with open ('not_found_associated_genes.txt', 'w') as txtfile:
  for protein, gene_dict in proteins_to_genes_dict.items():
    gene = gene_dict['Gene']
    if gene == 'None':
      txtfile.write (f'{protein}, \t {gene} \n')
    else:
      protein_name = uniprot_protein_name_dict[protein]['Protein Name']
...
new_csv_file (column_labels = ['UniProt Acc', 'Gene Name', 'Protein Name'], column_data = [proteins, genes, protein_names], filename = 'Protein and Gene names.csv', index_status = False)
...
thymosin_interactors_interactors_dict = protein_interactors_counter (thymosin_interactors_interactors, total_hippie_pairs)
print (thymosin_interactors_interactors_dict)
...
network_nodes (thymosin_related_interactors_list, total_thymosin_pairs, 'protein', 'autophagosome_thymosin.csv')
